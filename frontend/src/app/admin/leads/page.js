...
export default function LeadsPage() {
  ...
  // selectedTier: null = all, "untiered" = untiered, number = specific tier
  ...
  // Derived boolean — true only when selectedTier is exactly "untiered"
  const isUntiered = selectedTier === "untiered";
  ...
  const statCards = [
    ...
    {
      label: isUntiered
          ? "Contacts in Tier Untiered"
          : selectedTier
              ? `Contacts in Tier ${selectedTier}`
              : "Total Contacts",
      sublabel: isUntiered
          ? "Filtered to tier untiered"
          : selectedTier
              ...
              <div style={modalOverlay}>
                <div style={modalContent}>
                  <div style={modalHeader}>
                    <h2 style={{ margin: 0, fontSize: "18px", color: TEXT_PRIMARY }}>
                      Import Lead Data
                    </h2>
                    <X
                        size={20}
...
// Styles
const pageContainer = { padding: "32px", background: BG_DARK, minHeight: "100vh" };
...
const statLabel = { fontSize: "12px", fontWeight: 700, color: TEXT_SECONDARY, textTransform: "uppercase", letterSpacing: "0.05em" };
const statSub = { fontSize: "11px", color: TEXT_SECONDARY, margin: "4px 0 0" };
const statValue = { fontSize: "30px", fontWeight: 800 };
const headerActionRow = { display: "flex", justifyContent: "space-between", marginBottom: "24px", alignItems: "center" };
const tabsGroup = {
  display: "flex",
  gap: "8px",
  background: BG_DARK,
